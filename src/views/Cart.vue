<template>
    <div>
      <!--顶部导航-->
      <nav-header></nav-header>
      <!--面包屑-->
      <nav-bread>
        <!--插槽-->
        <span slot="Goods">我的购物车</span>
        <!-- <span slot="b">测试一下</span>-->
      </nav-bread>
      <!--样式svg-->
      <svg style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1"
           xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        <defs>
          <symbol id="icon-add" viewBox="0 0 32 32">
            <title>add2</title>
            <path class="path1"
                  d="M15 17h-13.664c-0.554 0-1.002-0.446-1.002-1 0-0.552 0.452-1 1.002-1h13.664v-13.664c0-0.554 0.446-1.002 1-1.002 0.552 0 1 0.452 1 1.002v13.664h13.664c0.554 0 1.002 0.446 1.002 1 0 0.552-0.452 1-1.002 1h-13.664v13.664c0 0.554-0.446 1.002-1 1.002-0.552 0-1-0.452-1-1.002v-13.664z"></path>
          </symbol>
          <symbol id="icon-ok" viewBox="0 0 32 32">
            <title>ok</title>
            <path class="path1"
                  d="M31.020 0.438c-0.512-0.363-1.135-0.507-1.757-0.406s-1.166 0.435-1.529 0.937l-17.965 24.679-5.753-5.67c-0.445-0.438-1.035-0.679-1.664-0.679s-1.219 0.241-1.664 0.679c-0.917 0.904-0.917 2.375 0 3.279l7.712 7.6c0.438 0.432 1.045 0.681 1.665 0.681l0.195-0.008c0.688-0.057 1.314-0.406 1.717-0.959l19.582-26.9c0.754-1.038 0.512-2.488-0.538-3.233z"></path>
          </symbol>
          <symbol id="icon-edit" viewBox="0 0 32 32">
            <title>edit</title>
            <path class="path1"
                  d="M28.287 8.51l-4.805-4.806 0.831-0.831c0.472-0.472 1.086-0.777 1.564-0.777 0.248 0 0.452 0.082 0.622 0.253l3.143 3.144c0.539 0.54 0.133 1.529-0.524 2.186l-0.831 0.831zM26.805 9.992l-1.138 1.138-4.805-4.806 1.138-1.138 4.805 4.806zM24.186 12.612l-14.758 14.762-4.805-4.806 14.758-14.762 4.805 4.806zM7.379 28.288l-4.892 1.224 1.223-4.894 3.669 3.67zM31.123 4.011l-3.143-3.144c-0.567-0.567-1.294-0.867-2.103-0.867-1.036 0-2.174 0.52-3.045 1.391l-20.429 20.436c-0.135 0.134-0.23 0.302-0.276 0.487l-2.095 8.385c-0.089 0.355 0.017 0.736 0.276 0.995 0.198 0.198 0.461 0.307 0.741 0.307 0.085 0 0.171-0.010 0.254-0.031l8.381-2.096c0.185-0.047 0.354-0.142 0.487-0.276l20.43-20.436c1.409-1.41 2.042-3.632 0.524-5.15v0z"></path>
          </symbol>
          <symbol id="icon-del" viewBox="0 0 32 32">
            <title>delete</title>
            <path class="path1"
                  d="M11.355 4.129v-2.065h9.29v2.065h-9.29zM6.194 29.935v-23.742h19.613v23.742h-19.613zM30.968 4.129h-8.258v-3.097c0-0.569-0.463-1.032-1.032-1.032h-11.355c-0.569 0-1.032 0.463-1.032 1.032v3.097h-8.258c-0.569 0-1.032 0.463-1.032 1.032s0.463 1.032 1.032 1.032h3.097v24.774c0 0.569 0.463 1.032 1.032 1.032h21.677c0.569 0 1.032-0.463 1.032-1.032v-24.774h3.097c0.569 0 1.032-0.463 1.032-1.032s-0.463-1.032-1.032-1.032v0z"></path>
            <path class="path2"
                  d="M10.323 9.806c-0.569 0-1.032 0.463-1.032 1.032v14.452c0 0.569 0.463 1.032 1.032 1.032s1.032-0.463 1.032-1.032v-14.452c0-0.569-0.463-1.032-1.032-1.032z"></path>
            <path class="path3"
                  d="M16 9.806c-0.569 0-1.032 0.463-1.032 1.032v14.452c0 0.569 0.463 1.032 1.032 1.032s1.032-0.463 1.032-1.032v-14.452c0-0.569-0.463-1.032-1.032-1.032z"></path>
            <path class="path4"
                  d="M21.677 9.806c-0.569 0-1.032 0.463-1.032 1.032v14.452c0 0.569 0.463 1.032 1.032 1.032s1.032-0.463 1.032-1.032v-14.452c0-0.569-0.463-1.032-1.032-1.032z"></path>
          </symbol>
          <symbol id="icon-clock" viewBox="0 0 32 32">
            <title>clock</title>
            <path class="path1" fill="#605f5f"
                  d="M16 29c-7.168 0-13-5.831-13-13s5.832-13 13-13c7.168 0 13 5.832 13 13s-5.832 13-13 13zM16 0c-8.822 0-16 7.178-16 16s7.178 16 16 16c8.822 0 16-7.178 16-16s-7.178-16-16-16z"></path>
            <path class="path2" fill="#605f5f"
                  d="M23.958 21.837l-6.958-6.489v-6.282c0-0.827-0.673-1.5-1.5-1.5s-1.5 0.673-1.5 1.5v6.934c0 0.414 0.174 0.814 0.477 1.098l7.435 6.934c0.279 0.259 0.642 0.402 1.023 0.402 0.415 0 0.814-0.174 1.096-0.477 0.564-0.605 0.532-1.555-0.073-2.12z"></path>
          </symbol>
        </defs>
      </svg>


      <div class="container">
        <div class="cart">
          <div class="page-title-normal">
            <h2 class="page-title-h2"><span>我的购物车</span></h2>
          </div>
          <div class="item-list-wrap">
            <div class="cart-item">
              <div class="cart-item-head">
                <ul>
                  <li>商品</li>
                  <li>单价</li>
                  <li>数量</li>
                  <li>小计</li>
                  <li>操作</li>
                </ul>
              </div>
              <ul class="cart-item-list">
                <li v-for="item in cartList">

                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascipt:;"
                         class="checkbox-btn item-check-btn"
                         v-bind:class="{'check':item.checked=='1'}"
                         @click="editCart('checked', item)">
                        <svg class="icon icon-ok">
                          <use xlink:href="#icon-ok"></use>
                        </svg>
                      </a>
                    </div>
                    <div class="cart-item-pic">
                      <img v-lazy="'/static/' + item.productImage" v-bind:alt="item.productName">
                    </div>
                    <div class="cart-item-title">
                      <div class="item-name">{{item.productName}}</div>
                    </div>
                  </div>

                  <div class="cart-tab-2">
                    <div class="item-price">{{item.salePrice|currency('$')}}</div>
                    <!--<div class="item-price">{{item.salePrice}}</div>-->
                  </div>

                  <!--商品数量-->
                  <div class="cart-tab-3">
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" @click="editCart('minu',item)">-</a>
                          <span class="select-ipt" >{{item.productNum}}</span>
                          <a class="input-add" @click="editCart('add',item)">+</a>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="cart-tab-4">
                    <div class="item-price-total">{{(item.productNum*item.salePrice) |currency('￥')}}</div>
                    <!--<div class="item-price-total">{{(item.productNum*item.salePrice)}}</div>-->
                  </div>

                  <div class="cart-tab-5">
                    <div class="cart-item-opration">
                      <!--删除按钮-->
                      <a href="javascript:;" class="item-edit-btn"
                         @click="delCartConfirm(item.productId,item.productNum)">
                        <svg class="icon icon-del">
                          <use xlink:href="#icon-del"></use>
                        </svg>
                      </a>
                    </div>
                  </div>

                </li>
              </ul>
            </div>
          </div>
          <div class="cart-foot-wrap">
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascipt:;"
                     @click="toggleCheckAll">
                    <span class="checkbox-btn item-check-btn"
                          v-bind:class="{'check':checkAllFlag}">
                        <svg class="icon icon-ok">
                          <use xlink:href="#icon-ok"/>
                        </svg>
                    </span>
                    <span>全选</span>
                  </a>
                </div>
              </div>
              <div class="cart-foot-r">
                <div class="item-total">
                 <!-- 总价: <span class="total-price">{{totalPrice |currency('￥')}}</span>-->
                  <!-- 使用了过滤器 -->
                  总价: <span class="total-price">{{totalPrice |currency('￥')}}</span>
                </div>
                <div class="btn-wrap">
                  <!--判断购物车商品数量 如果为零则无法点击-->
                  <a class="btn btn--red" v-bind:class="{'btn--dis':checkedCount==0}" @click="checkOut">去结算</a>
                  <!--<a href="javascript:;" class="btn btn&#45;&#45;red">去结算</a>-->
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!--模态框 提示删除-->
      <Model :mdShow="modalConfirm" @close="closeModal">
        <p slot="message">确认删除此商品吗</p>
        <div slot="btnGroup">
          <a href="javascript:;"
             class="btn btn--m"
             @click="delCart">确认</a>
          <a href="javascript:;"
             class="btn btn--m"
             @click="modalConfirm=false">取消</a>

        </div>
      </Model>

      <!--底部-->
      <nav-footer></nav-footer>
    </div>
</template>
<style>
.select-self-area{
  display: flex;
  background: #fff;

}
.input-sub,.input-add{
  background: #f0f0f0;
  border-radius: 3px;
  min-width: 50px;
  max-width: 240px;
  height: 30px;
  line-height: 30px;
}
.select-self-area .select-ipt{
    padding: 0;
    text-align: center;
  }
</style>

<script>

  /*取名字注意 不要跟原有的h5标签冲突 虽然不会报错 但是会发出警告*/
  import NavHeader from '@/components/Header.vue'
  import NavFooter from '@/components/Footer.vue'
  /*面包屑*/
  import NavBread from '@/components/Navbread.vue'
  /*模态框 登录框和遮罩层*/
  import Model from  '@/components/Model.vue'
  /*axios*/
  import axios from "axios"

  /*金额格式化
  因为在util中的js 输出的是有名字的 非匿名 所以需要用 {} 来包住 写在这局部过滤器 */
  // 项目为全局导入
  // import {currency} from '@/util/currency.js'

  export default {
    name: "Cart",
    data () {
      return {
        //购物车商品列表
        cartList:[],
        //删除购物车商品 模态框
        modalConfirm:false,
        //商品id缓存 以便传参
        productId:"",
        //全选svg图标按钮 默认不标识
        //checkAllFlag:false
        //商品数量
        amount:""
      }
    },
    components:{
      NavHeader,
      NavFooter,
      NavBread,
      Model,
    },
    mounted () {
      this.init()
    },

    //过滤器 定义是 值是函数
   /* filters:{
      //货币格式化 接收一个值 对其进行一系列的处理 处理完后 返回一个全新的值
      currency :currency

    },*/

    //计算属性
    // 可用于快速计算视图（View）中显示的属性。这些计算将被缓存，并且只在需要时更新
    computed:{
      //全局计算 全选状态点击改变后
      // 如不用计算属性 则每次状态改变后都需要 调用循环拿到更改后的数据 再把全选视图勾上 操作麻烦

      //computed用来监控自己定义的变量，该变量不在data里面声明，直接在computed里面定义，然后就可以在页面上进行双向数据绑定展示出结果或者用作其他处理；原来在 data 中 checkAllFlag 是有定义的

      //判断购物车的数量 和 选中的数量 一样 即为全选
      checkAllFlag () {
        //注意 在 computed 中  是计算的属性来使用(checkedCount),而非是函数
        return this.checkedCount == this.cartList.length

      },
      //获得商品数量
      checkAmount () {
        return this.checkedCount
      },
      //选中的数量 i返回的值 假设为5 等同于 this.cartList.length 假设为5 全选
      checkedCount () {
        var i=0;
        this.cartList.forEach( (item,index,arr)=>{
          if(item.checked=='1'){
            i++
          }else {
            i--
          }
        });
        return i

      },
      //总价格计算
      totalPrice () {
        var money=0;
        this.cartList.forEach( (item,index,arr)=>{
          if(item.checked=='1'){
            //单价 X 数量
            money += parseFloat(item.salePrice) * parseInt(item.productNum)
          }
        });
        return money
      }


    },

    methods:{
      //获取购物车商品列表
      init (){
        axios.get('/users/cartList')
          .then((response)=>{
            let res=response.data;
            this.cartList=res.result;
          })
      },

      /* 取消x图标*/
      closeModal () {
        this.modalConfirm=false
      },

      /*删除按钮svg图标*/
      delCartConfirm (productId,amount) {
        //保存 productId 以便删除确认按钮获取 传参
        this.productId=productId;
        this.amount=amount;
        this.modalConfirm=true
      },

      /*模态框确认删除按钮*/
      delCart () {
        axios.post('/users/cartDel',{
          productId:this.productId
        }).then( (response)=>{
          let res=response.data
          if(res.status=='0'){
            this.modalConfirm=false
            this.init();
            this.$store.commit('updateCartCount',-this.amount)
          }
        })
      },

      /*商品数量的 加减 以及 单选*/
      editCart (flag,item) {


        /*if(flag=='add'){
          item.productNum++;
        }else if(flag=='minu'){
          if(item.productNum<=1){
            return;
          }
          item.productNum--;
        }else if(flag=='checked'){
          item.checked = item.checked=="1"?'0':'1';

          if(item.checked==false){
            item.productNum=item.productNum-(item.productNum)
          }else {

          }
          /*
          我的思路是获取 item.checked 默认选中时的数量 让一个变量接受储存
          但是又不能改变 item.productNum 的数量 又需要让
          this.$store.commit('updateCartCount',num) 变为需要的数字 还要被数据库储存
          */


        if(flag=='add'){
          item.productNum++;
        }else if(flag=='minu'){
          if(item.productNum<=1){
            return;
          }
          item.productNum--;
        }else{
          item.checked = item.checked=="1"?'0':'1';
        }

        axios.post('/users/cartEdit',{
          productId:item.productId,
          productNum:item.productNum,
          checked:item.checked
        }).then( (response)=>{
          let res=response.data;

         /* let num=0
          if(flag=='add'){
            num=1
            this.$store.commit('updateCartCount',num)
          }else if(flag=='minu'){
            num=-1
            this.$store.commit('updateCartCount',num)
          }else if(flag=='checked'){
            if(item.checked==false){
              console.log(-(item.productNum))
              this.$store.commit('updateCartCount',-(item.productNum))
            }else {
              console.log(+(item.productNum))
              this.$store.commit('updateCartCount',+(item.productNum))
            }
          }*/

          let num=0
          if(flag=='add'){
            num=1
          }else if(flag=='minu'){
            num=-1
          }
          this.$store.commit('updateCartCount',num)
        })

      },

      /*全选按钮*/
      toggleCheckAll () {
        //无法赋值的 上面 computed已经 定义 要么true 要么false 无法在取反之间相互切换 所以 不能用一下的 !this.checkAllFlag;
        //this.checkAllFla 实时属性的计算 无法再被赋值 并不是定义在data
        //this.checkAllFlag=!this.checkAllFlag;

        // 取反 自由切换

        console.log(this.checkAllFlag)


        var flag= !this.checkAllFlag


        this.cartList.map( (item,index,arr) =>{
          // item 单件商品的所有数据
          //console.log(item)
          // 下标
          //console.log(index)
          item.checked=flag?'1':'0';



          axios.post('/users/editCheckAll',{
            checkAll:flag
          }).then((response)=>{
            let res=response.data
            if(res.status=='0'){
              console.log('updata suc')
            }
          })
        })


      },

      // 结算按钮 防止更改class样式
      checkOut () {
        if(this.checkedCount>0){
          this.$router.push({
            path:'/address'
          })
        }else {
          alert('修改样式也是不行的')

        }
      }

    }

  }
</script>

